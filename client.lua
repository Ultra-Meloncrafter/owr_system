...
function checkIfPitIsEntered()
    while true do
        ...
        atPitEntry = PitEntry:isPointInside(coord)
        if atPitEntry then
            return true
        else
            return false
...
function checkIfPitIsLeft()
    while true do
        local plyPed = PlayerPedId()
        local coord = GetEntityCoords(plyPed)
        atPitExit = PitExit:isPointInside(coord)
        if atPitExit then
            return true
        else
            return false
...
function checkIfDRSZoneIsEntered()
    while true do
        local plyPed = PlayerPedId()
        local coord = GetEntityCoords(plyPed)
        atDRSEntry = DRSEntry:isPointInside(coord)
        if atDRSEntry then
            return true
        else
            return false
...
function checkIfDRSZoneIsLeft()
    while true do
        local plyPed = PlayerPedId()
        local coord = GetEntityCoords(plyPed)
        atDRSExit = DRSExit:isPointInside(coord)
        if atDRSExit then
            return true
        else
            return false
...
function checkIfFinishLaneIsCrossed()
    while true do
        local plyPed = PlayerPedId()
        local coord = GetEntityCoords(plyPed)
        atFinishLane = FinishLane:isPointInside(coord)
        if atFinishLane then
            return true
        else
            return false
        end
        ...
        if IsPedInAnyVehicle(playerPed, false) then
            ...
            for i = 1, #OWRVeh do
                if GetHashKey(OWRVeh[i]) == model then
                    if exports["ultra_utility"]:ifDRSEntered() and not DRSEntered then  
                        ...
                        StartScreenEffect('RaceTurbo', 0, 1)
                    elseif exports["ultra_utility"]:ifDRSLeft() and DRSEntered then
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(playerPed, false) then
            ...
            for i = 1, #OWRVeh do
                if GetHashKey(OWRVeh[i]) == model then
                    if exports["ultra_utility"]:ifPitEntered() and not PitEntered and pitRequested then  
                        ...
                        SlowDownToLimitSpeed(playerVeh, speed)
                        SetVehicleMaxSpeed(playerVeh, speed)
                    elseif exports["ultra_utility"]:ifPitLeft() and PitEntered then
